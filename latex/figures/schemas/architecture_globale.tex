...
\draw[arrow] (risk.south) -- ++(0,-1.1cm) -| (agents.north);
\draw[arrow] (agents) -- (supervisor);
\draw[arrow] (supervisor) -- (human);
% Le rejet suit un chemin séparé sous la ligne principale.
\draw[danger] (risk.south east) -- ++(0,-0.45cm)
  -| node[pos=0.5,below]{rejet ou anomalie}
  ([yshift=-0.45cm]optimizer.south) -- (optimizer.south);
\end{tikzpicture}
\end{document}
