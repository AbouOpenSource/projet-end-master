...
\node[control, below=of action] (owner) {Responsable\\superviseur, risque,\\conformité ou IT};
\draw[arrow] (metrics) -- (threshold);
\draw[arrow] (finance) -- (threshold);
\draw[arrow] (threshold) -- (action);
\draw[arrow] (action) -- (owner);
\draw[danger] (action.south) to[bend right=35] node[label,below]{retour stratégie de référence} (finance.south);
\end{tikzpicture}
\end{document}
